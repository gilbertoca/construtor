 ...
 * @author gilberto
 */
public class ViewParking extends BorderPage {

    private static final long serialVersionUID = 1L;
    protected Form form = new Form("form");
    protected Table table = new Table("table");
    protected PageLink editLink = new PageLink("editLink", EditParking.class);
    ...
    public ViewParking() {
        ...
        getModel().put("title", getMessage("viewParking.title"));
        getModel().put("heading", getMessage("viewParking.heading"));
        ...
        column = new Column("parkingSpace");
        table.addColumn(column);

        //editLink.setImageSrc("/assets/images/table-edit.png");
        editLink.setParameter("referrer", "/view-parking.htm");
        ...
        table.setDataProvider(new DataProvider<Parking>() {

            public List<Parking> getData() {
                return (List<Parking>) findByName();
            }
        });
    }
    ...
    private List findByName() {
        System.out.println("\n findByName() method \n");
        Query queryObject = em.createNamedQuery("Parking.findByAddress");
        queryObject.setParameter("address", "%" + nameField.getValue() + "%");
    ...
    public boolean onNewClick() {
        System.out.println("\n onNewClick() method \n");
        String path = getContext().getPagePath(EditParking.class);
        path += "?referrer=/view-parking.htm";
        setRedirect(path);
        return false;
    }
    ...
    private void delete(Long id) throws Exception {
        ...
        try {
            ...
            em.remove(em.find(Parking.class, id));
